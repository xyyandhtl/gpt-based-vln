...
def main(opts):
    ...
    print(f'default_gpu: {default_gpu}, n_gpu: {n_gpu}, device: {device}')
    ...
    grad_norm = 0
    # task2loss：一个字典，为每个训练任务创建一个 RunningMeter 对象，用于记录每个任务的损失值。
    # n_examples：一个默认值为 0 的字典，用于记录每个任务处理的样本数量。
    # n_in_units：一个默认值为 0 的字典，用于记录每个任务处理的输入单元数量。
    # n_loss_units：一个默认值为 0 的字典，用于记录每个任务的损失单元数量。
    # grad_norm：初始化梯度范数为 0。
