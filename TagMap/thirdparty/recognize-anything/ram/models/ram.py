...
class RAM(nn.Module):
    ...
    def override_class_threshold(self, cls, threshold):
        assert (type(cls) == str) and (type(threshold) == float)
        try:
            cls_idx = int(np.where(cls == self.tag_list)[0])
        except:
            raise ValueError('{} not in the tag list'.format(cls))
        assert (
                threshold >= 0.0 and threshold <= 1.0
            ), "threshold must be between 0 and 1"
        self.class_threshold[cls_idx] = threshold
    ...
    def generate_tag_openset(self,
        ...
        # TODO also return tag confidences!!!
